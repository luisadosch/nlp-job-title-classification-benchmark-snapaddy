...
The prompt-engineering mode relies on the Google Gemini API and therefore requires a valid API key. For security reasons, the key is not hard-coded but must be provided via a Streamlit secrets variable during deployment, using a file \texttt{dashboard/.streamlit/secrets.toml} with the entry
\begin{verbatim}
