...
\begin{itemize}
    \item \textbf{Prompt 1:} when my train accuracy is 99% and my test accuray 32% what can i do. i predict departments from jobittels (different language) and use MODEL_CKPT = "xlm-roberta-base
    \item \textbf{Prompt 2:}"CV Confusion Matrix (counts) - trained on augumenteted data" improve this title
    \item \textbf{Prompt 3:}"which Huggingface model would you recommend me to use to classify job titles in different languages into departments and seniority levels?"
    \item \textbf{Prompt 4:} "explain to me the difference between macro f1 and weighted f1"
    \item \textbf{Prompt 5:} in the end i will have a second notebook for department. for both notebooks i will do a readme file so it is easier to understand what i did. can oyu improve this readme and make markdown out of it:...."
    \item \textbf{Prompt 6:}based on my comments, improve this read me (also include the main results in the end as a markdown table):Fine-Tuning Models for Seniority and Department Prediction
    \item \textbf{Prompt 7:} Improve this text for our report so it is wirtten in correct english : ..."
\end{itemize}
